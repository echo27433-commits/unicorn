  ...
  loading: () => (
    <div className="flex h-full min-h-[360px] items-center justify-center rounded-2xl bg-black md:min-h-[480px]">
      <div className="flex flex-col items-center gap-3">
        <span className="h-8 w-8 animate-spin rounded-full border-2 border-[#FFAA00] border-t-transparent" />
...
export default function OurLocation() {
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute -right-32 top-0 h-96 w-96 rounded-full bg-[#FFAA00]/[0.06] blur-3xl" />
      ...
      <div className="relative mx-auto w-full max-w-7xl px-5 py-20 md:px-8 md:py-28 lg:px-10 lg:py-32">
        ...
        >
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#FFAA00]">
            Our Location
          </p>
          <h2 className="text-glow-white mt-4 text-4xl font-bold leading-[1.08] tracking-tight text-white md:text-5xl lg:text-6xl">
            Global presence,{" "}
            <span className="text-glow-orange text-[#FFAA00]">local expertise</span>
          </h2>
        ...
        <div
          className={`mt-12 h-px w-full origin-left bg-gradient-to-r from-[#FFAA00] via-[#FFAA00]/40 to-transparent transition-transform duration-1000 md:mt-16 ${
        ...
        >
          <div className="flex flex-col gap-3 lg:col-span-2">
              ...
              return (
                <button
                  ...
                  className={`group w-full rounded-2xl border p-5 text-left transition-all duration-500 md:p-6 ${
                    active
                      ? "border-[#FFAA00]/50 bg-[#FFAA00]/10 shadow-[0_8px_32px_-8px_rgba(255,170,0,0.25)]"
                      : "border-white/10 bg-white/[0.03] hover:border-[#FFAA00]/30 hover:bg-[#FFAA00]/[0.05]"
                  } ${revealed ? "translate-x-0 opacity-100" : "-translate-x-6 opacity-0"}`}
                  style={{ transitionDelay: revealed ? `${index * 80}ms` : "0ms" }}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#FFAA00]">
                        {loc.timezone}
                      </p>
                      <h3
                        className={`mt-1 text-xl font-bold tracking-tight transition-colors md:text-2xl ${
                          active ? "text-[#FFAA00]" : "text-white group-hover:text-[#FFAA00]"
                        }`}
                      >
                        {loc.country}
                      </h3>
                    ...
                    <span
                      className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold transition-all duration-300 ${
                        active
                          ...
                          : "bg-white/10 text-white/70 group-hover:bg-[#FFAA00]/20 group-hover:text-[#FFAA00]"
                      }`}
                    >
                      {index + 1}
                    </span>
            ...
            <button
              ...
              className="mt-1 text-left text-sm font-medium text-[#FFAA00] underline-offset-4 transition hover:underline"
            >
              View all locations
            </button>
          </div>
